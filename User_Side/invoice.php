...
$result = $conn->query($sql);
	while($row = $result->fetch_assoc()){
		...
		if($row["quantity"] > $row2["product_quantity"]){
			echo "<script>alert('Order of Product " . $row["product_id"] . " has exceed total quantity. (Current Stock: " . $row2["product_quantity"] . "). Please order equal or less to this.')</script><br>";
			$checksum = 1;
		}
	}
}else{
